...
class E9API {
	...
	/*async updatePost(id: number, post: {
		...
		return phin({
			method: "POST",
			url: "https://e926.net/post/update.json",
			data: {
				id,
				post,
				reason
	...
	getPostById(id: number, fetchImage?: false): Promise<E926Post>;
	getPostById(id: number, fetchImage?: true): Promise<{ image: Buffer; post: E926Post }>;
	...
	async getPostByMD5(md5: string, fetchImage?: boolean) {
		...
		return phin({
			method: "GET",
			url: `https://e926.net/post/check_md5.json?md5=${md5}`,
			headers: {
				"User-Agent": this.userAgent
			}
	...
	/*async flagPost(id: number, flagOption: "uploader" | "inferior" | 1 | 2 | 3 | 4 | 5 | 6, inferiorParent?) {
		return phin({
			method: "GET",
			url: `https://e926.net/post/flag.json?id=${id}&flag_option=${flagOption}${flagOption === "inferior" ? `&inferior_parent=${inferiorParent}` : ""}`,
			headers: {
	...
	/*async deletePost(id: number, reason: string, permanent: boolean): Promise<boolean> {
		return phin({
			method: "GET",
			url: `https://e926.net/post/delete.json?id=${id}&reason=${reason}`,
			headers: {
				"Authorization": this.apiKey,
				"User-Agent": this.userAgent
			},
			...
			if (permanent) return phin({
				method: "GET",
				url: `https://e926.net/post/delete.json?id=${id}&reason=${reason}&mode=1`,
				headers: {
					"Authorization": this.apiKey,
					"User-Agent": this.userAgent
				},
